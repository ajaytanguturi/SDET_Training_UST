...
test("To Filter out the trains based on the Departure time range", async({page})=>{
    ...
    //To verify the departure filter is working fine or not 
    await page.getByRole('button', { name: 'Departure time range', exact: true }).click();
    await page.locator("//div[text()='Afternoon']").nth(0).click();
