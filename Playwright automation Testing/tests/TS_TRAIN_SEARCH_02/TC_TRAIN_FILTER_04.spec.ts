...
test ("To Verify the Quota filter section to filter based on the given requirements", async({page})=>{
    ...
    // To verify the Filter section quota is working or not 
    await page.getByRole('button', { name: 'Quota', exact: true }).click();
    await page.locator("//div[contains(@aria-label, 'Senior')]").click();
